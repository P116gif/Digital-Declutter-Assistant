...
def _verify_token(credentials: HTTPAuthorizationCredentials):
    ...
    try:
        payload = jwt.decode(credentials.credentials, PUBLIC_KEY, algorithms=[ALGORITHM], options={"verify_exp": True})
...
async def delete_image(image_id: str, credentials: HTTPAuthorizationCredentials = Security(security)):
    token_image_id = _verify_token(credentials)
